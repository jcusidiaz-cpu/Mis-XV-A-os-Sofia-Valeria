...
const SHEETS_URL = "https://script.google.com/macros/s/AKfycbyvDPrigICG8_GUNYExGEsfyWP9-r1g7InGzE6BZcIEr7xkxCifCmoozL7SxVodx3zedA/exec";
  ...
  "sello.png": `
    <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 100%; height: auto; display: block; margin: 0 auto; outline: none;">
      ...
      <g fill="#d0af58">
        ...
        <circle cx="65" cy="59" r="3.5" />
        <circle cx="100" cy="49" r="4.5" />
        <circle cx="135" cy="59" r="3.5" />
  ...
  document.querySelectorAll(".hoja img").forEach(img => {
    ...
    if (fallbackSvg) {
      ...
      const replaceWithSvg = () => {
        if (!img.parentNode || img.parentNode !== parent) return;
        const div = document.createElement("div");
